...
## Corpus Check
- 148 files · ~7,103,326 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 620 nodes · 715 edges · 70 communities (61 shown, 9 thin omitted)
...
5. `run_backtest()` - 7 edges
6. `build_synthetic_dataset()` - 7 edges
7. `export_to_onnx()` - 7 edges
8. `export_saabas_topologies()` - 7 edges
9. `command` - 7 edges
10. `scripts` - 7 edges
...
Cohesion: 0.09
Nodes (29): datetime, evaluate_predictions(), Any, ml/evaluation/evaluate_weekly.py — Auditoria Semanal de Erros, Acertos e Drift…, build_synthetic_dataset(), check_db_for_data(), generate_calibrated_row(), Any (+21 more)
...
### Community 12 - "location.ts"
Cohesion: 0.06
Nodes (18): BASEMAP_LABELS, BASEMAP_STYLES, BasemapStyle, CARTO_API_KEY, DEFAULT_BASEMAP, tileUrl(), GpsResult, renderedHtml (+10 more)

### Community 14 - "weather-cache.ts"
Cohesion: 0.10
Nodes (18): AetherInferenceClient, PredictionPoint, SaabasContribution, WorkerErrorMsg, WorkerOutMsg, WorkerReadyMsg, WorkerResultMsg, CacheCheckResult (+10 more)

### Community 17 - "calibrate.py"
Cohesion: 0.17
Nodes (19): apply_bias(), apply_isotonic(), boundary_weights(), compute_bias(), fit_isotonic(), _frontier_index_proxy(), ml/training/calibrate.py — Calibração de viés + pesos de fronteira (só TREINO).…, Aproxima o índice da faixa Boa/Moderada a partir da concentração. (+11 more)
...
Cohesion: 0.33
Nodes (5): 1. Formulação Matemática do Índice Individual ($I_p$), 2. Determinação do IQAr Consolidado Global, 3. Tabela Regulamentar de Faixas e Limiares, 4. Prescrições de Saúde Pública por Faixa, AetherML — 02 Resolução CONAMA 491/2018 e Cálculo do IQAr
...
## Knowledge Gaps
- **269 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `name`, `version`, `type` (+264 more)
...
- **Why does `globalIQAr()` connect `inference.worker.ts` to `location.ts`?**
